...
# Let's evaluate how many time we draw a random double number equal to its
# float conversion.
...
######################################################################
# We finally check that double operations between float numpers remain
# floats.
...
# Let's imagine now we want to define an intervalle in which a double is
# converted to the same float. Let's find out about it length.
...
# Let's estimate
# :math:`\mathbb{P}\left(x_{64} < y_{64} \Longrightarrow x_{32} < y_{32} \; | \; |x-y| \leqslant d\right)`
# ?
...
# Let's estimate
# :math:`\mathbb{P}\left(x_{64} \leqslant y_{64} \Longrightarrow x_{32} \leqslant y_{32} \; | \; |x-y| \leqslant d\right)`
# ?
...
# comparison <http://www.crcummins.com/CRCProcessing.pdf>`_, ``<`` is
# 5-10% faster than ``<=``.
