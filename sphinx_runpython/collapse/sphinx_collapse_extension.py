...
class CollapseDirective(Directive):
    ...
    * *hide*: the text is shown by default unless this option is set up.
    ...
    def run(self):
        ...
        if "hide" in self.options and self.options["hide"] not in (
            False,
            "False",
            "false",
            0,
...
def visit_collapse_node_html(self, node):
    ...
    if node["show"]:
        content = f'<div id="collapse{nid}"">'
        label = hide
    else:
        content = f'<div id="collapse{nid}" style="display:none;">'
        label = unhide
    self.body.append(
        f'<p style="margin-bottom:10px;"><button id="colidb{nid}" '
        f'onclick="myFunction{nid}()">{label}</button></p>\n'
    )
